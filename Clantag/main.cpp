...
void main()
{
...

	{
		...
		FileClanTag.getline(cBuf, 64); // ñ÷èòûâàíèå çàäåðæêó äî "
		iDelay = std::atoi(cBuf);

		FileClanTag.getline(cBuf, 64, '"'); // ïåðåõîä äî ïåðâîé ", ïîñòàâèâ êóðñîð âïåðåä, ãäå áóäåò êëàíòåã
		while (!FileClanTag.eof())
		{
			FileClanTag.getline(cBuf, 64, '"'); // ñ÷èòûâàíèå êëàíòåãà äî "
			sClanTagList[i1] = cBuf;

			sClanTagCount++;
			i1++;

			FileClanTag.getline(cBuf, 64, '"'); // ïåðåõîä íà íîâóþ ñòðîêó äî ", ïîñòàâèâ êóðñîð âïåðåä, ãäå áóäåò êëàíòåã
		}
	}
